...
const Anous = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        const response = await fetch(
          `https://graphql.contentful.com/content/v1/spaces/${SPACE_ID}/environments/${ENVIRONMENT}`,
          {
            ...
            body: JSON.stringify({
              query: `
                {
                  anousCollection(limit: 5, order: date_DESC) {
                    items {
                      sys { id }
                      titel
                      paragraf { json }
        ...
        console.log(data); // لتتأكد أن البيانات وصلت
        setNews(data?.data?.anousCollection?.items || []);
      } catch (err) {
  ...
  return (
    <>
      {/* SEO */}
      <Helmet>
        <title>روضة أنوس | مؤسسة بنت الريف</title>
        <meta
          name="description"
          content="تعرف على آخر الأخبار والمستجدات الخاصة بروضة أنوس ضمن مؤسسة بنت الريف."
        />
      </Helmet>
              ...
              >
                ...
                <Box sx={{ mt: 1 }}>
                  <Button
                    ...
                    size="small"
                    component={Link}
                    to={`/news/${item.sys.id}?type=anous`}
                    sx={{
                      fontSize: "16px",
                      fontWeight: "bold",
                      color: "#eeb60f",
